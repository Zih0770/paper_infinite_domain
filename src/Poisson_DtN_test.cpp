...
// --- DtN header (mfemElasticity repository) ---
...
// --------- NEW: GSLIB single-point evaluator (only addition) ----------
static double GSLIB_EvalAtPoint(ParMesh &pmesh, const ParGridFunction &gf, const Vector &x)
{
    const int dim = pmesh.Dimension();
    Vector vxyz(dim);
    for (int d = 0; d < dim; ++d) { vxyz(d) = x(d); }
...
static void SaveOnSmallDomainSubmesh(const std::string &tag, int order, int small_attr, ParMesh &pmesh, const ParGridFunction &phi_shifted, const ParGridFunction &exact_shifted)
{
    ...
    if (has_local)
    {
        ...
        if (subrank == 0)
        {
            ...
            info << "glvis_np " << subsize << "\n"
                 << "glvis_cmd glvis -np " << subsize << " -m results/" << tag << ".sm.mesh" << " -g results/" << tag << ".sm.resid\n";
            std::cout << "[GLVis] submesh writers (np) = " << subsize << "\n"
                      << " glvis -np " << subsize << " -m results/" << tag << ".sm.mesh" << " -g results/" << tag << ".sm.resid\n";
        }
    ...
    if (Mpi::WorldRank() == 0)
    {
        ...
        rL2 << std::setprecision(16) << "relative_L2_error_on_small_domain " << relL2 << "\n";
    }
}
...
int main(int argc, char *argv[])
{
    ...
    args.AddOption(&lmax, "-lmax", "--lmax", "DtN spherical-harmonic cutoff (ℓmax).");
    ...
    // ------------------------------ Assembly --------------------------------
    Vector rho_vals(pmesh.attributes.Max());
    rho_vals = 0.0;
    ...
    */
    ...
    if (dim == 2)
    {
        // Use OrthoSolver wrapper (as in mfemElasticity example)
    ...
    // --------------------------- Postprocessing ------------------------------
    auto Tpost0 = clk::now();
    // Always report value at c0 (now via GSLIB)
    const double r_eps = 0.02;
    const int nsmp = 12;
    // CHANGED HERE: use GSLIB for center value
    double phi_c = GSLIB_EvalAtPoint(pmesh, phi, c0);
    ...
    // CHANGED HERE: use GSLIB for center value of exact
    double exact_c0 = GSLIB_EvalAtPoint(pmesh, exact_gf, c0);
    if (myid == 0)
    {
        std::cout<<"phi_ex at centre: "<<exact_c0<<std::endl;
    }
    //exact_gf -= exact_c0;
    ...
    tag_ss << std::filesystem::path(mesh_file).stem().string() << "_DtN_lmax" << lmax;
